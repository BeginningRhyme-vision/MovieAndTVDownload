"""
按 tmdbId + season + episode 从 SubDL 为剧集补外挂字幕。

输入：success.jsonl（download_tv.py 的成功日志，含 tmdbId/season/episode/year）
输出：R2 上 {folder_prefix}/{year}/{tmdbId}/S{ss}/E{ee}/subs/{lang}.vtt + .srt
     （s3.enabled=false 时退化为写本地同构目录）

字幕与视频落在**同一个集目录**下，与 download_tv.py 的
`{folder_prefix}/{year}/{tmdbId}/S{ss}/E{ee}/` 结构、以及 R2 对象键完全同构，
前端按同前缀一次列举即可拿到视频 + 元信息 + 字幕。

🔑 为什么以 R2 为准而不是扫本地目录：
   下载侧上传成功后会**连旁车资产带集目录一起删掉**（几十万集规模下，
   本地留副本会把 inode 吃干净）。所以本地目录不存在 ≠ 没有字幕 ——
   恰恰相反，那是最正常的状态。若按本地判重，会把每一集已有字幕的片
   都当成缺口重抓一遍，白烧 SubDL 配额，且抓来的字幕只落本地、永远进不了 R2。
   故：**已有语种问 R2，新字幕传 R2，meta.json 也就地更新**。

🔑 设计原则：**字幕是"可有可无"的附属物**。
   取到最好，取不到就算了——绝不因为字幕而让任何集被判失败、被重下、被重传。
   故本脚本：
     - 全程不抛异常到调用方（单集失败只记录，继续下一集）；
     - 无 API Key / 无 success.jsonl 时安静退出（退出码 0），不报错。

脚本可重复执行，已存在的字幕会跳过，因此新下载的剧集直接再跑一次即可。

🔑 与电影版的关键差异：**处理单位是一集**，配额压力大得多。
   一部剧几十上百集，每集都要独立查询与下载，而免费档每账号每天只有 50 次
   下载额度。故缺口台账（subtitles_gaps.jsonl）按集级 key 记录，且多 key
   轮换几乎是必需的。

🔑 可以与下载侧（download_tv.py）**同时运行**，无需错开：
   - 本脚本只处理 success.jsonl 里 uploaded=true 的集，而下载侧是先把视频 +
     meta.json 全部传进 R2 之后才写这条记录。两边操作的集合天然不相交；
   ...
   - R2 模式下本脚本不碰 downloads/ 目录、pending 文件与主流程锁，字幕只写
     临时目录、传完即删。
   注意 success.jsonl 在启动时一次性读入：运行期间新上传的集不会被本次处理，
   等它们落库后再跑一次即可（幂等）。
"""
...
# download_tv 模块级只读 config、不建连接、不起线程，import 是安全的。
sys.path.insert(0, str(Path(__file__).resolve().parent))
import download_tv as dt  # noqa: E402  (import 时已把同目录 .env 载入环境变量)
...
def _config_api_key():
    ...
    刻意不复用 dt.load_config()：那个函数写死了只返回 download_tv 段。
...
#
# 支持**多账号轮换**：免费档每账号每天只有 50 次下载。剧集按**集**计数，
# 一部 20 集的剧两种语言就要 40 次——单 key 一天补不完一部剧，多 key 几乎
# 是必需的。额度按**账号**计而非按 IP 计，故多 key 轮换确实能叠加额度。
...
SUCCESS_LOG = dt.SUCCESS_LOG
STATE_LOG = str(_SCRIPT_DIR / "subtitles.jsonl")
# 【缺口台账】记录"SubDL 确认没有某集某语种"，下次运行直接跳过这个语种。
#
# 为什么剧集比电影更需要它：SubDL 的剧集字幕覆盖远不如电影，尤其是中文与
# 冷门剧；没有台账时每次重跑都把这些必然空手而归的集重查一遍，而配额本就
# 稀缺（免费档每账号 50 次/天，还要按集摊）。
#
...
# 目录结构、语种白名单、输出格式全部复用 download_tv（单一事实来源）：
# 补来的字幕必须与下载侧落在同一个地方、用同一套格式，否则前端按同前缀
# 列举时会拿到对不上的东西。要改语种/格式请改 download_tv.assets 段。
SUBTITLE_LANGUAGES = dt.SUBTITLE_LANGUAGES
SUBTITLE_FORMATS = dt.SUBTITLE_FORMATS

# SubDL 的 language 字段用大写语言代码（"EN"/"ZH"/"BR_PT"），查询参数也要
# 用同一套。config 里写的是 IETF 风格的小写码（en / zh / zh-cn / pt-br），
# 两者不是简单的 upper()：
#   - SubDL 对 ZH-CN/ZH_CN 宽松匹配为 ZH，但 PT_BR 的结果 language 字段是
#     "BR_PT"——不做映射的话 pick_candidates 永远比不上；
#   - **未知码不会报错，而是返回全语种结果**，只靠"查到了东西"判断不出
#     配置写错了。
# 故：先查别名表，再按 ^[A-Z]{2,3}(_[A-Z]{2})?$ 校验；不合法的语种启动时
# 告警并整体跳过（不进 pending、不进台账），而不是每集都白查一遍。
...
# 响应里没有 Retry-After 头，只能自定步长。
THROTTLE_COOLDOWN = 30
THROTTLE_MAX_ROUNDS = 3
# 同一语种最多试几个候选：解压后发现是 ASS/SSA、zip 里没有字幕文件、或多集包
# 里挑不出本集时换下一条。每试一条都消耗一次下载额度，所以不能无限试。
...
# 还没进 success.jsonl 的集，与我们处理的集天然不相交），撞上也只需重读一次。
META_UPDATE_RETRIES = 3
META_UPDATE_BACKOFF = 0.5
# 防御性上限：字幕 zip 正常只有几十 KB，源站返回异常内容时不能整个读进内存。
MAX_ZIP_BYTES = dt.SUBTITLE_MAX_BYTES

state_lock = threading.Lock()

# 单实例锁：cron 与手动同时触发时，两个实例会各自搜索同一批集、各自消耗
...
# 是否把字幕写进 R2。下载侧开了上传就必须走 R2 —— 本地集目录早被删了。
REMOTE_MODE = dt.S3_ENABLED

# SubDL 免费账号每天只能**下载** 50 个字幕（搜索不受限）。额度用尽后所有下载
# 一律 429，且当天不会恢复。响应体形如：
...
def _redact(text):
    ...
    搜索接口的 key 就在 query 里。一条 429 报错就会把 `api_key=subdl_xxx`
    原样写进日志文件——日志经常要贴出来排查，等于泄露凭证。
    所有对外打印的异常文本都必须过这一道。
    """
...
def episode_key(tmdb_id, season, episode):
    """集级唯一 key：`{tmdbId}_S{season:02d}E{episode:02d}`。与下载侧同源。"""
    return dt.episode_key(tmdb_id, season, episode)


def subs_dir(tmdb_id, season, episode, year):
    """字幕目录：{集目录}/subs —— 与视频、meta.json 同级。

    仅在 s3.enabled=false 的纯本地模式下作为最终落点；R2 模式下字幕写临时
    目录、传完即删，不走这里。
    """
    return os.path.join(
        dt.episode_dir(tmdb_id, season, episode, year), dt.SUBS_SUBDIR
    )
...
def _language_of(filename):
    ...
    ⚠️ 只认 SUBTITLE_FORMATS 里的扩展名（srt/vtt）。若把 ASS 也算"已有"，
    该语种就被一个前端 <track> 根本认不得的文件永久锁死、永不再补。
    ASS 不落盘，但 R2 上万一有旧残留仍要按"没有"处理。
    """
...
def _list_remote_subtitles(tmdb_id, season, episode, year):
    """列举 R2 上该集 subs/ 下已有的字幕文件名（不含目录）。

    返回 (文件名列表, 是否查询成功)。查询失败时返回 ([], False) —— 调用方
    据此**跳过这一集**，而不是当作"没有字幕"去重抓：列举失败多半是网络抖动，
    ...
    prefix = dt.build_s3_key(
        tmdb_id, season, episode, year, dt.SUBS_SUBDIR
    ) + "/"
    label = episode_key(tmdb_id, season, episode)
    try:
        client = dt.get_s3_client()
        names = []
        paginator = client.get_paginator("list_objects_v2")
        for page in paginator.paginate(Bucket=dt.S3_BUCKET, Prefix=prefix):
            for obj in page.get("Contents", []):
                ...
                if name and "/" not in name:
                    names.append(name)
        return names, True
    except Exception as exc:  # noqa: BLE001 - 列举失败只跳过这一集
        print(f"  [{label}] ⚠️ 列举 R2 字幕失败（已跳过）: {exc}", flush=True)
        return [], False
...
def _update_remote_meta(tmdb_id, season, episode, year, new_files):
    ...
    成功上传的事实。
    """
    key = dt.build_s3_key(tmdb_id, season, episode, year, "meta.json")
    label = episode_key(tmdb_id, season, episode)
    try:
        client = dt.get_s3_client()
    except Exception as exc:  # noqa: BLE001
        print(f"  [{label}] ⚠️ 连接 R2 失败（字幕已上传，跳过 meta 更新）: {exc}",
              flush=True)
        return False, 0

    for attempt in range(META_UPDATE_RETRIES):
        try:
            obj = client.get_object(Bucket=dt.S3_BUCKET, Key=key)
            etag = obj.get("ETag")
            meta = json.loads(obj["Body"].read().decode("utf-8"))
        except Exception as exc:  # noqa: BLE001 - 没有 meta 就不更新，不是错误
            print(
                f"  [{label}] ⚠️ 读取 meta.json 失败（字幕已上传，跳过更新）: "
                f"{exc}", flush=True,
            )
            return False, 0
        ...
        try:
            ...
            if _is_precondition_failed(exc) and attempt < META_UPDATE_RETRIES - 1:
                ...
                print(
                    f"  [{label}] meta.json 被并发修改，重读重试"
                    f"（第 {attempt + 1} 次）",
                    flush=True,
                )
                time.sleep(META_UPDATE_BACKOFF * (attempt + 1))
                continue
            print(f"  [{label}] ⚠️ 回写 meta.json 失败（字幕已上传，"
...
def load_entries():
    ...
    🔑 R2 模式下**只要 uploaded=true 的集**：下载侧对上传失败的集也会写一条
    uploaded=false（语义是"下载成功但还没进 R2"，等 reupload 补传）。给这种集
    传字幕会在 R2 上造出「有 subs/ 却没有视频」的畸形目录——前端按前缀列举
    会拿到一集放不了的片。等它补传成功后再跑一次本脚本即可补上字幕。
    """
    entries = {}
    if not os.path.exists(SUCCESS_LOG):
        print(f"未找到 {SUCCESS_LOG}（还没有下载成功的剧集），无需补字幕",
              flush=True)
        return []
    ...
    with open(SUCCESS_LOG, "r", encoding="utf-8") as file:
        for line in file:
            ...
            season = dt.parse_int(data.get("season"))
            episode = dt.parse_int(data.get("episode"))
            key = episode_key(tmdb_id, season, episode)
            if not key:
                continue

            if REMOTE_MODE and not data.get("uploaded"):
                # 同一集的旧记录可能已进 entries（success.jsonl 按集去重覆盖
                # 写，理论上不会，但坏数据下要保证"未上传"是终态判定）。
                entries.pop(key, None)
                skipped_not_uploaded += 1
                continue

            # year 直接取记录里的首播年份：它就是目录那一层的来源，不再从
            # final_path 反解目录名（上传成功后本地视频已被删除，但目录结构
            # 由 (tmdbId, season, episode, year) 唯一决定，重建即可）。
            entries[key] = {
                "key": key,
                "tmdbId": tmdb_id,
                "season": season,
    ...
    if skipped_not_uploaded:
        print(
            f"跳过 {skipped_not_uploaded} 集尚未上传 R2 的剧集"
            f"（补传成功后再跑一次本脚本即可补字幕）",
            flush=True,
        )
...
class QuotaExhausted(Exception):
    ...
    两者若都落进 result["missing"]，打印出来都是「已保存 [] 缺失 ['zh']」，
    完全同形——一次额度耗尽的空跑会被误判成"补字幕功能坏了"。

    它还是**确定性**失败：对**当前这个 key** 而言重试、换集都没用，当天就是
...
def request_with_retry(method, url, **kwargs):
    ...
    # 单列一类上抛，调用方才能与"这集源站没有"区分开（后者是业务错误）。
    if last_status == 429:
...
def search_subtitles(tmdb_id, season, episode, languages=None):
    """按 tmdb_id + 季 + 集**逐语种**查询候选字幕，合并返回。

    为什么不一次 languages="EN,ZH" 混查：SubDL 只返回首页（subs_per_page
    上限 30），且结果按热度排，英文字幕动辄几十上百条，会把中文挤出首页。
    混查时 ZH 可能一条都剩不下 —— 而 pick_candidates 找不到就会把该语种判成
    missing 写进缺口台账，从此**永久放弃**。这是一个静默且不可逆的误判，
    所以每个语种单独查一次（搜索不耗下载额度，只多几次请求）。

    type 恒为 "tv"：本脚本的输入全部来自 download_tv.py 的 success.jsonl，
    处理单位就是剧集。SubDL 用 type 决定拿 tmdb_id 去查哪张表，电影与剧集是
    两个独立的编号空间，用错表要么查不到、要么查到一部完全不相干的片。

    SubDL 语义：影片不存在 → status=false + "can't find movie or tv"；
    影片存在但该语种没字幕 → status=true + 空列表。前者对任何语种都一样，
    遇到即整集放弃（上抛 RuntimeError，调用方按错误消息归类）。
    """
...
def _episode_range(item):
    ...
    ep_from = dt.parse_int(item.get("episode_from"))
    ep_end = dt.parse_int(item.get("episode_end"))
    if ep_from is None or ep_end is None or ep_from == ep_end:
        return None
    return (min(ep_from, ep_end), max(ep_from, ep_end))


def pick_candidates(subtitles, language, season, episode):
    """挑出该语种覆盖本集的候选，按 SubDL 给的顺序（相关度）返回。

    过滤三关（剧集特有，电影版没有后两关）：
      1. 语种一致（按 subdl_language_code 映射后比较）；
      2. 排除 full_season 整季包 —— 那是一个装着全季的 zip，且往往没有
         可靠的集标记；
      3. 季一致 + 本集被覆盖。**多集包**（episode_from != episode_end）的
         `episode` 字段只是起始集，必须按 [from, end] 范围判断，不能拿
         `episode` 去精确比对，否则一个 E01-E10 的合集会被误判成只有 E01。
    条目缺 season/episode 字段时信任服务端已按查询参数过滤。

    返回一小串候选而不是一条：结果里**没有格式字段**，下载解压后才知道是不是
    ASS（前端 <track> 不认、我们也不落盘），多集包还可能解出来没有本集。
    只挑一条的话这些情况都会把该语种误判成 missing 进台账、从此不再补。
    上限 MAX_CANDIDATES_PER_LANGUAGE，每试一条都烧一次下载额度。
    """
    ...
    for item in subtitles:
        ...
        if item.get("full_season"):
            continue
        item_season = dt.parse_int(item.get("season"))
        ...
        if ep_range is not None:
            ...
            item_episode = dt.parse_int(item.get("episode"))
...
def pick_best(subtitles, language, season, episode):
    ...
    picked = pick_candidates(subtitles, language, season, episode)
...
def _episode_name_pattern(season, episode):
    ...
    前后不能紧邻数字（否则 S01E03 会误命中 S01E030）。
    """
    return re.compile(
...
def extract_srt(zip_bytes, season=None, episode=None, require_match=False):
    """SubDL 下载回来的是 zip，从里面取出**本集**的字幕文件。

    返回 (正文字节, 扩展名)；没有可用字幕或体积异常时返回 (None, None)。

    与电影版的关键差异：zip 里可能装着多集（多集包），必须按文件名里的集标记
    挑出本集。给定 season/episode 时优先选带对应标记的文件；没有命中时，
    require_match=True（多集包）返回 (None, None) 以免**把别的集存成本集**，
...
def decode_subtitle(raw):
    ...
    """
    return dt._decode_subtitle(raw)
...
def _sniff_format(text, declared):
    ...
    为什么不能信扩展名（两个方向都出过事）：
      - 源站把 SRT 正文塞进声明 type="vtt" 的条目 —— 按 vtt 原样存下会得到
        一个缺 WEBVTT 头的 .vtt，浏览器 <track> 直接拒绝加载；
      - zip 里也有文件名与内容对不上的情况 —— ASS 正文若被当成 srt 处理，
        会连 .srt 带 .vtt 一起写废。
    正文特征是事实，文件名只是传闻。
    """
    head = (text or "").lstrip()
...
def _write_variants(target_dir, language, text, source_format):
    ...
    与下载侧同一套转换逻辑（dt.srt_to_vtt / dt.vtt_to_srt），保证落盘的格式与
    命名完全一致。source_format 是**不带点**的扩展名（srt / vtt / ass ...），
    仅作兜底提示 —— 真正的判据是正文（见 _sniff_format）。

    ass/ssa 等非 srt/vtt 格式**不落盘**，返回 []：前端 <track> 只认 vtt，
    转换规则又与 srt/vtt 完全不同。若原样存成 en.ass，该语种就会被一个用不了
    的文件"锁死"——_language_of 认为已有，永不再补。调用方应换下一条候选。
    """
    ...
    if fmt == "srt":
        variants["srt"] = text
        variants["vtt"] = dt.srt_to_vtt(text)
    else:
        variants["vtt"] = text
        variants["srt"] = dt.vtt_to_srt(text)
...
def _reconcile_meta(entry, remote_files, result):
    """本集不需要再抓任何字幕时，顺手核对 R2 上 meta.json 是否把已有字幕都记全了。
    ...
    标记，不改状态——字幕本身没缺，不该被报成"这集有问题"。
    """
    if not REMOTE_MODE or not remote_files:
        return result
    ok, added = _update_remote_meta(
        entry["tmdbId"], entry["season"], entry["episode"], entry.get("year"),
        remote_files,
    )
    if added:
...
def download_one(entry, ledger=None):
    ...
    # 全局闸门已落：后面的集一律直接跳过。继续跑只会让每集各发若干次注定
    # 失败的请求，白等重试间隔，还会把接口问题记成「源站缺这个语种」。
    # 如实沿用置位时的原因（额度耗尽 / 搜索限流），不笼统归成一种。
    if stop_fetching.is_set():
        return key, dict(stop_reason) or {"status": "quota_exhausted"}

    # 第一步：确定还缺哪些语种。R2 模式下**必须问 R2** —— 本地集目录在
    # 视频上传成功那一刻就被删了，扫本地只会得到空集，把每集都当成缺口。
    if REMOTE_MODE:
        remote_files, ok = _list_remote_subtitles(tmdb_id, season, episode, year)
        if not ok:
            return key, {"status": "list_failed"}
        existing = {lang for lang in map(_language_of, remote_files) if lang}
    else:
        remote_files = []
        # 集目录已被用户删掉的集没必要补字幕：否则会凭空造出一个只有
        # subs/ 的孤立目录，而 success.jsonl 只记录"下过"，不知道"还在不在"。
        try:
            folder = dt.episode_dir(tmdb_id, season, episode, year)
        except ValueError:
            return key, {"status": "failed", "error": "身份字段不全，无法定位集目录"}
        if not os.path.isdir(folder):
            return key, {"status": "local_missing"}
        existing = _existing_languages_local(
            subs_dir(tmdb_id, season, episode, year)
        )
    ...
    if not pending:
        return key, _reconcile_meta(entry, remote_files, {"status": "skipped"})

    # 查缺口台账：源站已经确认没有的语种不再问第二遍。这是省配额的关键 ——
    # 剧集的中文字幕缺口尤其大，每次重跑都重查等于把额度扔进水里。
    given_up = ledger.get(key, set()) if ledger else set()
    if given_up:
        remaining = [lang for lang in pending if lang.lower() not in given_up]
        if not remaining:
            # 该集所有还缺的语种都已确认源站没有 —— 这不是"补完了"，
            # 而是"没得补了"，状态上要与 skipped 区分开，否则统计会误导。
            return key, _reconcile_meta(
                entry, remote_files,
    ...
    # 集目录下的 subs/。两种模式下后续写盘逻辑完全一致。
    if REMOTE_MODE:
        try:
            target_dir = tempfile.mkdtemp(prefix=f"subs_{key}_")
        except OSError as exc:
            return key, {"status": "failed", "error": f"建临时目录失败: {exc}"}
    else:
        target_dir = subs_dir(tmdb_id, season, episode, year)
        try:
            os.makedirs(target_dir, exist_ok=True)
        except OSError as exc:
            return key, {"status": "failed", "error": f"建目录失败: {exc}"}

    try:
        return key, _fetch_into(entry, pending, target_dir)
    finally:
        if REMOTE_MODE:
...
def _download_candidate(picked, language, season, episode, target_dir):
    """下载一条候选并落盘，返回已写文件名列表。

    返回 []：候选**不可用**（zip 里没字幕、多集包里挑不出本集、或正文是
    ASS/SSA 等不落盘的格式），调用方应换下一条候选。网络/解压/写盘等
    **过程性**失败直接抛异常，由调用方归入 fetch_failed——两者必须分开：
    前者换候选还能救，后者下次重试才有意义。
    """
    ...
    # 多集包必须严格按集标记挑文件，挑不到宁可放弃（返回 []，换下一条候选），
    # 绝不能把别的集的字幕存成本集 —— 那种错误极难被发现。
    content, extension = extract_srt(
        b"".join(chunks), season, episode,
...
def _fetch_into(entry, pending, target_dir):
    ...
    try:
        ...
        subtitles = search_subtitles(tmdb_id, season, episode, pending)
    except SearchThrottled as exc:
        # 冷却多轮仍被限流：这一集**压根没查成**，与"SubDL 库里没有这个
        # tmdb_id"完全不同。混进 search_failed 会把那个统计弄脏（真查不到的
        # 集值得从待办里剔除，被限流的集下次还要再试）。
        _signal_stop("search_throttled", str(exc))
    ...
    #                  下次照常重试。
    # 混在一起的后果很隐蔽：一次网络抖动就会让那集的该语种被永久放弃，
    # 而日志上看起来只是"源站没有"，无从察觉。
    result = {"status": "ok", "saved": [], "missing": [], "fetch_failed": []}

    def _unattempted():
        """本集还没得出任何结论的语种——额度恢复后要接着试的就是这些。
    ...
    for language in pending:
        candidates = pick_candidates(subtitles, language, season, episode)
        if not candidates:
            # 源站没有该语种：这是常态，不是错误
        ...
        try:
            # 逐候选试：解压后发现是 ASS/SSA、zip 里没有字幕文件、或多集包里
            # 挑不出本集时换下一条。搜索结果里没有格式字段，只能下载后才知道；
            # 每试一条都烧一次额度，上限由 pick_candidates 控制。
            saved = []
            for picked in candidates:
                saved = _download_candidate(
                    picked, language, season, episode, target_dir
                )
                if saved:
                    break
            if saved:
                ...
                # 所有候选都不可用。归入 missing 而非 fetch_failed 是刻意的：
                # 候选选择是确定性的，下次重试会拿到**同一批** zip、得到同样的
                # 结果，只是白烧几次下载额度。
                result["missing"].append(language)
        except QuotaExhausted as exc:
            # 额度耗尽：置位全局闸门让后续剧集直接跳过，并**立刻停止本集**剩下
            # 的语种——它们同样一个都拿不到。
    ...
    if REMOTE_MODE and result["saved"]:
        uploaded = _upload_subtitles(entry, target_dir, result["saved"])
        # 上传失败的不算数：本地临时目录马上就删，没进 R2 等于这份字幕不存在，
        # 报成功会让人以为补上了，而下次运行仍会（正确地）再抓一遍。
        result["saved"] = uploaded
        if uploaded:
            ok, _ = _update_remote_meta(
                tmdb_id, season, episode, year, uploaded
            )
...
def _upload_subtitles(entry, target_dir, names):
    ...
    for name in names:
        ...
        if not os.path.isfile(local):
            continue
        try:
            s3_key = dt.build_s3_key(
                tmdb_id, season, episode, year, f"{dt.SUBS_SUBDIR}/{name}"
            )
            ok, reason = dt.upload_to_r2(local, s3_key)
            if ok:
                uploaded.append(name)
            else:
                print(f"  [{label}] ⚠️ 字幕上传失败（已跳过）{name}: {reason}",
                      flush=True)
        except Exception as exc:  # noqa: BLE001 - 单个字幕失败不影响其它
            print(f"  [{label}] ⚠️ 字幕上传异常（已跳过）{name}: {exc}",
                  flush=True)
    return uploaded


def load_ledger():
    """读缺口台账，返回 {集级key: {已确认源站没有的语种}}。

    纯追加文件，同一集可能有多行（每次运行各记一次）——按 key **并集** 合并
    （写入端保持纯追加，避免为了去重而重写整个文件、与并发写冲突）。
    ...
    try:
        with open(LEDGER_LOG, "r", encoding="utf-8") as file:
            for line in file:
                ...
                if not key or not isinstance(langs, list):
                    continue
                ledger.setdefault(key, set()).update(
...
def record_gap(entry, languages):
...
def acquire_lock():
    ...
    pid = _read_lock_pid()
    if pid and pid != os.getpid() and dt._pid_alive(pid):
        return False
    if pid:
        dt.remove_file(LOCK_FILE)
    try:
...
def _run():
    ...
    if bad_langs:
        ...
        # 结果，若不在这里拦下，这些语种会在每集上都被判成 missing 进台账。
        print(
    ...
    ledger = load_ledger()
    ledger_note = f"；缺口台账已记 {len(ledger)} 集" if ledger else ""
    print(
        f"待处理剧集: {len(entries)}（语种: {', '.join(SUBTITLE_LANGUAGES)}；"
        f"落点: {'R2 ' + dt.S3_BUCKET if REMOTE_MODE else '本地 ' + dt.BASE_DIR}；"
    ...
    # 是每条记录自带的，两者不总是同步（例如首集就被拦下的极端时序）。
    quota_reset_at = None
    blocked_kinds = set()
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        ...
        for index, future in enumerate(as_completed(futures), 1):
            ...
            except Exception as exc:  # noqa: BLE001 - 单集异常不能带塌整批
                key = entry["key"]
                result = {"status": "failed", "error": _redact(exc)}
                print(f"[{index}/{len(entries)}] {key} 异常（已跳过）: "
            ...
            if result.get("missing"):
                record_gap(entry, result["missing"])
            ...
            if result["status"] == "ok":
                ...
                print(
                    f"[{index}/{len(entries)}] {key} {entry.get('title')} "
                ...
                print(
                    f"[{index}/{len(entries)}] {key} {entry.get('title')} "
                    f"-> SubDL 库里没有这部剧，已记入台账 {result['missing']}",
                    flush=True,
                )
            elif result["status"] == "local_missing":
                print(
                    f"[{index}/{len(entries)}] {key} 本地集目录不存在"
                    f"（已删？），跳过",
                    flush=True,
                )
            elif result["status"] == "search_failed":
                print(
                    f"[{index}/{len(entries)}] {key} 查询失败（已跳过）: "
                    f"{result['error']}",
                    flush=True,
                )
                ...
                if not stop_announced and result.get("error"):
                    ...
                    print(
                        f"\n🛑 [{index}/{len(entries)}] {what}，剩余剧集全部跳过。\n"
                        f"   原因: {result['error']}\n"
                        f"   {when}\n"
                        f"   ⚠️ 这不是「源站没有字幕」——这些集一个都没查成，"
                        f"恢复后再跑一次本脚本即可继续补。",
                        flush=True,
                    )

    # 两种收场必须**说得明显不同**：
    #   跑完了  -> 圆满，所有待补的集都处理过了，没必要再跑；
    ...
    if stats.get("gap_confirmed"):
        print(
            f"其中 {stats['gap_confirmed']} 集因所缺语种此前已确认源站没有"
            f"而整集跳过（未消耗任何配额）",
            flush=True,
        )
    ...
    if blocked:
        ...
        print(
            ...
            f"{blocked} 集本次未处理（{reason}）。\n"
            f"   {nxt}（已补好的会自动跳过）。\n"
            f"   ⚠️ 剧集按**集**消耗额度（免费档每账号 50 次/天），"
            f"一部长剧往往要分多天补完。\n"
        ...
        # "都已尝试过"这话对被台账跳过的集不成立——它们这次一个请求都没发。
        # 收尾结论要经得起推敲，否则下次遇到"某集始终没字幕"会白排查一轮。
        skipped = stats.get("gap_confirmed", 0)
        scope = (
            f"   当前 success.jsonl 里的集都已处理过"
            f"（其中 {skipped} 集按缺口台账直接跳过），**无需今天再跑**；\n"
            if skipped else
            "   当前 success.jsonl 里的集都已尝试过，**无需今天再跑**；\n"
        )
        print(
            f"\n✅ ===== 全部待补剧集已处理完毕 =====\n"
            f"{spent}"
...
if __name__ == "__main__":
    ...
    # 流水线脚本）以为整条流水线出了问题。Ctrl+C 仍按惯例返回 130。
    try:
        main()
        ...
        print(f"字幕补全异常退出（不影响已下载的剧集）: {exc}", flush=True)
