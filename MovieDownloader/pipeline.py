...
#
# 🔴 2026-09-13 实测修正了原先的判断。此处曾写"取流是瓶颈（§12.3：取流 ~4.6
# 部/分钟 < 下载 ~6.4 部/分钟），故队列长期偏空、这个上限基本不会碰到"。
# 按实测链路上限 111MB/s 与成品中位数 2.08GB 重算：下载最多 **3.2 部/分钟**
# （总吞吐由带宽定死，与并发数无关）< 取流 4.6 —— **下载才是瓶颈**。
# ⇒ 队列会持续积压到上限、取流侧经常在 ENQUEUE_TIMEOUT 上等待，
#   这是**正常反压**而非故障。
#
# ⚠️ 队列里的每一条都是**等待被下载的签名直链**，而 vidlink 的 mp4 直链带时效。
# 容量越大 = 允许取流跑在下载前面越多 = 队首那条在队列里待得越久。
# 512 条按下载侧 3.2 部/分钟计，约需 2.7 小时才能消化完 —— 已接近签名时效的
# 安全边界，故**不要再调大**：调大只会让更多直链一起排队等到过期。
