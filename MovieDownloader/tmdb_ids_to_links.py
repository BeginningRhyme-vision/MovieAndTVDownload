...
# 跑满 max_rounds 仍未捞回的 ID 直接写 unresolved.txt、留给下次运行自动重试，
# **不再**在进程内做长冷却（曾有 30 分钟级的"加时赛"，已取消）。理由与下载侧一致：
# 下载侧已改为跨运行重试，取流线程若还在这里冷却半小时，pipeline 的下载侧早已
# 收工干等；而"代理额度耗尽 / enc-dec 维护"这类故障本就该由下次运行接力。
...
def _main_impl(argv=None, on_result=None, stop_event=None):
    ...
    while pending:
        ...
        if round_no >= max_rounds:
            ...
            # 打下来的。不在进程内再做长冷却重试（与下载侧的跨运行重试对齐），
            # 直接写 unresolved.txt，下次运行自动接力。
