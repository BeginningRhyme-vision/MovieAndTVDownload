...
def test_queue_capacity_stays_within_link_lifetime():
    ...
    队列里每一条都是**等待下载的签名直链**。队首那条最长要等多久，取决于
    **下载侧多久能把整队消化完** = 容量 ÷ 下载速率。

    ⚠️ 2026-09-13 修正：这里原先除的是**取流速率** 4.6 部/分钟，口径是错的——
    队列积压时取流侧本就被反压卡住，决定排空快慢的是**消费端**。
    按实测链路上限 111MB/s ÷ 成品中位数 2.08GB，下载最多 **3.2 部/分钟**
    （总吞吐由带宽定死，与 max_workers 无关）。用 3.2 而非 4.6 才是保守方向。

    vidlink 实测"签发 30h 后仍能 206 拉流"（§12.8）——但那只是"测到没失效"，
    **有效期上界始终未知**，故留大安全边际。

    阈值 6 小时相对当前值（512 ≈ 2.7h）仍有 2 倍余量；1024（5.3h）已逼近，
    再往上会被拦下——那种深度是拿时效换缓冲。
    """
    # 下载侧消费速率（部/分钟）：111MB/s ÷ 2.08GB/部。带宽定死，与并发数无关。
    download_rate_per_minute = 3.2
    hours_of_backlog = p.QUEUE_MAXSIZE / download_rate_per_minute / 60

    assert hours_of_backlog < 6, (
        f"队列容量 {p.QUEUE_MAXSIZE} 需 {hours_of_backlog:.1f} 小时才能消化完，"
        f"队首直链可能等到过期——这正是 §12 要消除的问题。"
        f"要提速只能加带宽或加机器，不是加深队列。"
    )
